    ...
    <form action="{{ route('health-certificates.update', $healthCertificate) }}" method="POST">
        ...
        <div class="card-body">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <x-tom-select name="pet_id" id="pet_id" :value="old('pet_id', $healthCertificate->pet_id)" required>
                            @foreach($pets as $pet)
                                <option value="{{ $pet->id }}" {{ old('pet_id', $healthCertificate->pet_id) == $pet->id ? 'selected' : '' }}>{{ $pet->name }}</option>
                            @endforeach
                        </x-tom-select>
                    </div>
                </div>
            ...
            <div class="form-group">
                ...
                <textarea name="clinical_notes" id="clinical_notes" rows="3" class="wysiwyg form-control">{{ old('clinical_notes', $healthCertificate->clinical_notes) }}</textarea>
            </div>
            <div class="form-group">
                <label for="notes">Informações Adicionais</label>
                <textarea name="notes" id="notes" rows="2" class="wysiwyg form-control">{{ old('notes', $healthCertificate->notes) }}</textarea>
            </div>
        </div>
