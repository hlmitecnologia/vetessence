@extends('layouts.app', ['title' => 'Editar Vacina'])
...
<form action="{{ route('vaccinations.update', $vaccination) }}" method="POST" class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div><label class="block text-sm font-medium text-gray-700 mb-1">Próxima Dose</label><input type="date" name="next_date" value="{{ $vaccination->next_date?->format('Y-m-d') }}" class="w-full px-4 py-2 border rounded-lg"></div>
