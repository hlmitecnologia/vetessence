    ...
    <form action="{{ route('treatment-plans.update', $plan) }}" method="POST">
        ...
        <div class="card-body">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <x-tom-select name="pet_id" id="pet_id" :value="old('pet_id', $plan->pet_id)" required>
                            @foreach($pets as $pet)
                                <option value="{{ $pet->id }}" {{ old('pet_id', $plan->pet_id) == $pet->id ? 'selected' : '' }}>{{ $pet->name }} - {{ $pet->tutors->first()->name ?? 'Sem tutor' }}</option>
                            @endforeach
                        </x-tom-select>
                        @error('pet_id')
            ...
            <div class="form-group">
                ...
                <textarea name="description" id="description" rows="2" class="wysiwyg form-control @error('description') is-invalid @enderror">{{ old('description', $plan->description) }}</textarea>
            ...
            <div class="row mt-3">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <textarea name="vet_notes" id="vet_notes" rows="2" class="wysiwyg form-control @error('vet_notes') is-invalid @enderror">{{ old('vet_notes', $plan->vet_notes) }}</textarea>
