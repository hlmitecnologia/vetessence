            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <x-tom-select name="pet_id" id="pet_id" :value="old('pet_id', $boarding->pet_id)" required>
                            @foreach($pets as $pet)
                                <option value="{{ $pet->id }}" {{ old('pet_id', $boarding->pet_id) == $pet->id ? 'selected' : '' }}>{{ $pet->name }} - {{ $pet->tutors->first()->name ?? 'Sem tutor' }}</option>
                            @endforeach
                        </x-tom-select>
