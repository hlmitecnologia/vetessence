    ...
    <form action="{{ route('imaging-exams.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                <textarea name="findings" id="findings" rows="4" class="wysiwyg form-control @error('findings') is-invalid @enderror">{{ old('findings') }}</textarea>
                            @error('findings')<span class="invalid-feedback">{{ $message }}</span>@enderror
            </div>
            <div class="form-group">
                <label for="impression">Impressão</label>
                <textarea name="impression" id="impression" rows="3" class="wysiwyg form-control @error('impression') is-invalid @enderror">{{ old('impression') }}</textarea>
                            @error('impression')<span class="invalid-feedback">{{ $message }}</span>@enderror
            </div>
            <div class="form-group">
                <label for="recommendations">Recomendações</label>
                <textarea name="recommendations" id="recommendations" rows="3" class="wysiwyg form-control @error('recommendations') is-invalid @enderror">{{ old('recommendations') }}</textarea>
