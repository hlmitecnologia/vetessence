...
class TecnospeedProvider implements NfseProvider
{
    protected string $baseUrl;

    public function __construct()
    {
        $this->baseUrl = config('nfse.tecnospeed.base_url', 'https://api.tecnospeed.com.br');
    }
    ...
    public function emitir(NfseConfig $config, Invoice $invoice): NfseResult
    {
        ...
        if (!$response->successful()) {
            return NfseResult::error(
                $body['erro'] ?? $body['error'] ?? 'Erro ao emitir NFSe via Tecnospeed',
                $body
            );
        }
    ...
    public function consultar(NfseConfig $config, string $nfseNumber): NfseResult
    {
        ...
        if (!$response->successful()) {
            return NfseResult::error(
                $body['erro'] ?? $body['error'] ?? 'Erro ao consultar NFSe via Tecnospeed',
                $body
            );
        }
    ...
    public function cancelar(NfseConfig $config, string $nfseNumber, string $motivo): NfseResult
    {
        ...
        if (!$response->successful()) {
            return NfseResult::error(
                $body['erro'] ?? $body['error'] ?? 'Erro ao cancelar NFSe via Tecnospeed',
                $body
            );
        }
    ...
    {
        return [
            'Authorization' => "Bearer {$config->tecnospeed_token}",
