...
class NfseWebhookControllerTest extends TestCase
{
    ...
    public function test_webhook_updates_only_matching_nfse_number()
    {
        $branch = Branch::factory()->create();
        $nfse1 = NfseInvoice::factory()->create([
            'branch_id' => $branch->id,
            'status' => 'pending',
            'nfse_number' => '111111',
            'issuance_date' => null,
        ...
        $this->postJson('/api/webhooks/nfse/' . $branch->id, [
            'nfse_number' => '111111',
            'status' => 'issued',
        ]);

        $this->assertEquals('issued', $nfse1->fresh()->status);
        $this->assertEquals('pending', $nfse2->fresh()->status);
    }
}
